...
public final class PdeMain {
    ...
    private static InetSocketAddress getOrchestratorAddress(final SystemTracker systemTracker) {
        final SrSystem orchestrator = systemTracker.getSystem(ApiConstants.ORCHESTRATOR_SYSTEM_NAME);
        if (orchestrator == null) {
            terminate("Could not find Orchestrator in the Service Registry.");
        }
        return orchestrator.getAddress();
    }
    ...
    public static void main(final String[] args) {
        ...
        systemTracker.start()
            .flatMap(result -> {
                ...
                final InetSocketAddress orchestratorAddress = getOrchestratorAddress(systemTracker);
            ...
            .onFailure(e -> {
                terminate("Failed to launch Plant Description Engine.", e);
            });
    }
